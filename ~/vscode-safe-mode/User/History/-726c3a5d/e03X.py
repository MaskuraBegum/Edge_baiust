...
def senrollment(request):
    if request.method == 'POST':
        form = EnrollmentForm(request.POST)  # Pass POST data to the form
        if form.is_valid():
            form.save()  # Save the valid form to the studentEnrollment model
    ...
    # Render the form regardless of POST/GET request
