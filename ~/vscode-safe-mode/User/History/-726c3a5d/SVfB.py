...
def senrollment(request):
    if request.method == 'POST':
        form = EnrollmentForm(request.POST)  # Initialize form with POST data
        if form.is_valid():
            form.save()
            messages.success(request, "Please wait for our email for successful enrollment")
            return redirect('home')
        else:
            else:
            # Debugging: Print form errors
            print(form.errors)
            messages.error(request, "Failed to enroll. Please correct the errors.")
    else:
        form = EnrollmentForm()  # Initialize an empty form for GET requests
    
    # Render the form regardless of whether the request is POST or GET
